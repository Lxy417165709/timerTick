...
#define gic_max_rd (getMaxRedistributorIndex())


// 下面的代码其实都可以重构...


uint32_t getMaxRedistributorIndex() {
	uint32_t index = 0;
...
// 为了符合单一职责，我修改了这个函数，将获取rd的最大索引独立出来。
// understand
void setGICAddr(void* dist, void* rdist)
{
  ...
  gic_addr_valid = 1;	// 一个标志位，表示是否初始化了
...
uint32_t enableGIC(void)
{
  ...
  if (gic_addr_valid==0) return 1;

  // First set the ARE bits
  //  gic_dist->GICD_CTLR = (1 << 5) | (1 << 4);	// 注释了还能正常工作..
...
// GICR_TYPER[1] 保留了一个 affinity值，所以我们通过 affinity 我们就可以确定是哪个redistrubutor
// GICR_XXX 这是属于redistrubutor的寄存器
// 每一个core的affnity值可以通过MPDIR_EL1寄存器获取， 每一个affinity占用8bit.
// 配置对应core的MPIDR值，可以将中断路由到该core上。
// 在单处理器系统中，ARM建议该寄存器的每个Aff <n>字段返回值0。
// GICR_TYPER[1]的值是与此redistrubutor关联的PE的身份。
// understand
uint32_t getRedistID(uint32_t affinity)
{
  ...
  // 循环遍历，把每个rd的affinity和传入的affinity进行比较，如果相等就说明找到了 对应affinity的rd
  do {
	// GICR_TYPER[1] 表示 GICR_TYPER寄存器的后32位.
...
uint32_t wakeUpRedist(uint32_t rd)
{
  ...
  // P668 有
  // 这3句代码就是将 ChildrenASleep位置0，且保持其他位不变
...
// 中断使能，由于中断分类不一样，导致处理方式不一样，所以代码有些相似
uint32_t enableInt(uint32_t ID, uint32_t rd)
{
  ...
  if (ID < 31)
  {
    ...
    if (rd > gic_max_rd)
    {
     ...
     // SGI or PPI
     ID   = ID & 0x1f;    // ... and which bit within the register，这一步感觉没必要，因为ID < 31 了
  ...
  else if (ID < 1020)
  {
    ...
    gic_dist->GICD_ISENABLER[bank] = ID; // GICD_ISENABLER 是寄存器组，每个寄存器控制一部分中断
  }
...
uint32_t setIntGroup(uint32_t ID, uint32_t rd, uint32_t security)
{
  ...
  if (ID < 31)
  {
    ...
    // Read current values
    group = gic_rdist[rd].sgis.GICR_IGROUPR[0];	// 资料说该寄存器的功能是：控制相应的SGI或PPI在组0或组1中。
