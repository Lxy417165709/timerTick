...
getCNTFRQ:	
  MRS     x0, CNTFRQ_EL0    // CNTFRQ_EL0 存储了频率，其是可读写的
  RET
...
setCNTFRQ:	
  MSR     CNTFRQ_EL0, x0  // CNTFRQ_EL0 存储了频率，其是可读写的
  RET
...
getVirtualCount:
  ISB
  MRS     x0, CNTVCT_EL0	// 虚拟计数值，其和物理计数值好像还有关系
  RET
...
getNSEL1PhysicalTimerValue:	
  MRS     x0, CNTP_TVAL_EL0	// 非安全的计数值（上面的代码还有一个是安全的，本次时钟采用安全的）
  RET
